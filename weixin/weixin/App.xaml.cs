...
namespace weixin
{
    public partial class App : Application
    {
        ...
        {
            Global.failedReferer = new List<string>();
             Global.failedFakeId = new List<string>();
             Global.isFirstPrepare =true;
            Global.isFirstLaunch = 1;
            Global.isDel = 0;
            Global.hasVCode = 0;
            Global.launchTimes = 0;
            Global.newAddMessage="0";
            Global.personMessageShouldRefresh = 0;
            Global.allInfo = new List<PersonMessage>();
            Global.isFirstSetupOrUpdate = 0;
            Global.send = false;
            Global.hasNewPeople = false;

            //1,wp 4,weixin  dic<weixin wp>
            Global.emoji = new Dictionary<string, string>();
            Global.weixin = new Dictionary<string, string>();
            StreamResourceInfo resource = App.GetResourceStream(new Uri("design/emotions/emoji.txt", UriKind.Relative));
            StreamReader sr = new StreamReader(resource.Stream);
            string line = sr.ReadLine();
            while (!String.IsNullOrEmpty(line))
            {
                string []array=line.Split(':');
                Global.emoji.Add(array[0], array[1]);
                line = sr.ReadLine();
            }
            sr.Close();

            resource = App.GetResourceStream(new Uri("design/emotions/weixin.txt",UriKind.Relative));
            sr = new StreamReader(resource.Stream);
           line = sr.ReadLine();
           while (!String.IsNullOrEmpty(line))
           {
               string[] array = line.Split('_');
               if(!Global.weixin.ContainsKey(array[0]))
                     Global.weixin.Add(array[0],array[2]);
               Global.weixin.Add(array[1], array[2]);
               line = sr.ReadLine();
           }
           sr.Close();
